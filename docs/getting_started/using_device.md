...
rdmnet_init() is called. It is safe to call any RDMnet API function from any callback.
...
{
  RdmnetPhysicalEndpointResponder new_responder;
...
Note that the RDMnet library requires the control field (and optional binding UID) received in the
DISC_MUTE message from a physical responder, so that controllers can fetch this information.
...
RdmnetPhysicalEndpointResponder responders[3];
...
std::vector<rdmnet::PhysicalEndpointResponder> responders = {
