...
public:
  ...
  etcpal::Error SendData(ScopeHandle scope_handle, const etcpal::Uuid& dest_cid, uint16_t manufacturer_id,
                         uint16_t protocol_id, const uint8_t* data, size_t data_len);
...
inline etcpal::Error EptClient::SendData(ScopeHandle scope_handle, const etcpal::Uuid& dest_cid,
...
/// \return #kEtcPalErrSys: An internal library or system call error occurred.
inline etcpal::Error EptClient::SendStatus(ScopeHandle scope_handle, const etcpal::Uuid& dest_cid,
                                           ept_status_code_t status_code, const char* status_string)
{
  ETCPAL_UNUSED_ARG(scope_handle);
  ETCPAL_UNUSED_ARG(dest_cid);
