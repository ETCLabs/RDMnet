...
inline const char* RptClientEntry::TypeToCString() const noexcept
{
  return rdmnet_rpt_client_type_to_string(type);
}

/// Get the client's RPT client type (controller or device) as a string.
inline std::string RptClientEntry::TypeToString() const
{
  return rdmnet_rpt_client_type_to_string(type);
}
