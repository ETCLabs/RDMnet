...
static void device_handle_rdm_command(const RdmCommandHeader* rdm_header,
                                      const uint8_t*          data,
                                      uint8_t                 data_len,
                                      RdmnetSyncRdmResponse*  response);
static void act_on_rdmnet_change_if_necessary(const RdmCommandHeader* rdm_header,
                                              RdmnetSyncRdmResponse*  response,
                                              bool                    llrp);
...
static void device_rdm_cmd_received(rdmnet_device_t         handle,
                                    const RdmnetRdmCommand* cmd,
                                    RdmnetSyncRdmResponse*  response,
                                    void*                   context);
static void device_llrp_rdm_cmd_received(rdmnet_device_t        handle,
...
void device_rdm_cmd_received(rdmnet_device_t         handle,
                             const RdmnetRdmCommand* command,
                             RdmnetSyncRdmResponse*  response,
                             void*                   context)
{
  ETCPAL_UNUSED_ARG(handle);
  ...
  if (command->rdm_header.command_class == kRdmCCSetCommand &&
      response->response_action == kRdmnetRdmResponseActionSendAck)
  {
    act_on_rdmnet_change_if_necessary(&command->rdm_header, response, false);
  }
}

void device_llrp_rdm_cmd_received(rdmnet_device_t        handle,
                                  const LlrpRdmCommand*  command,
                                  RdmnetSyncRdmResponse* response,
                                  void*                  context)
{
  ETCPAL_UNUSED_ARG(handle);
  ...
  if (command->rdm_header.command_class == kRdmCCSetCommand &&
      response->response_action == kRdmnetRdmResponseActionSendAck)
  {
    act_on_rdmnet_change_if_necessary(&command->rdm_header, response, true);
  }
}

void device_handle_rdm_command(const RdmCommandHeader* rdm_header,
                               const uint8_t*          data,
                               uint8_t                 data_len,
                               RdmnetSyncRdmResponse*  response)
{
...
void act_on_rdmnet_change_if_necessary(const RdmCommandHeader* rdm_header, RdmnetSyncRdmResponse* response, bool llrp)
{
  if (rdm_header->param_id == E133_COMPONENT_SCOPE)
  {
    RdmnetScopeConfig new_scope_config;
    new_scope_config.scope = default_responder_get_scope();
    default_responder_get_static_broker_addr(&new_scope_config.static_broker_addr);

    etcpal_error_t change_res =
        rdmnet_device_change_scope(device_state.device_handle, &new_scope_config,
                                   llrp ? kRdmnetDisconnectLlrpReconfigure : kRdmnetDisconnectRptReconfigure);
    if (change_res == kEtcPalErrOk)
    {
      etcpal_log(device_state.lparams, ETCPAL_LOG_INFO, "Device scope changed to '%s'", new_scope_config.scope);
    }
    else
    {
      etcpal_log(device_state.lparams, ETCPAL_LOG_ERR, "Device change scope failed: %s", etcpal_strerror(change_res));
      RDMNET_SYNC_SEND_RDM_NACK(response, kRdmNRHardwareFault);
    }
  }
  else if (rdm_header->param_id == E133_SEARCH_DOMAIN)
  {
    etcpal_error_t change_res =
        rdmnet_device_change_search_domain(device_state.device_handle, default_responder_get_search_domain(),
                                           llrp ? kRdmnetDisconnectLlrpReconfigure : kRdmnetDisconnectRptReconfigure);
    if (change_res == kEtcPalErrOk)
    {
      etcpal_log(device_state.lparams, ETCPAL_LOG_INFO, "Device search domain changed to '%s'",
                 default_responder_get_search_domain());
    }
    else
    {
      etcpal_log(device_state.lparams, ETCPAL_LOG_ERR, "Device change search domain failed: %s",
                 etcpal_strerror(change_res));
      RDMNET_SYNC_SEND_RDM_NACK(response, kRdmNRHardwareFault);
    }
  }
}
