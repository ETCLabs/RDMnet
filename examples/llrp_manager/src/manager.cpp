...
bool LlrpManagerExample::Startup(const etcpal::Logger& logger)
{
        ...
        if (line.length() >= 2)
        {
          switch (line[1])
          {
            ...
            case 'f':
              ...
              {
                int target_handle = std::stoi(line.substr(3));
                SetFactoryDefaults(target_handle);
              }
              catch (const std::exception&)
              {
...
void LlrpManagerExample::SetDeviceLabel(int target_handle, const std::string& label)
{
  ...
  if (mgr_pair != managers_.end())
  {
    ...
    if (target != targets_.end())
    {
      uint8_t label_truncated_size = (label.size() > 32 ? 32 : static_cast<uint8_t>(label.size()));
...
void LlrpManagerExample::SetResetDevice(int target_handle)
{
  auto mgr_pair = managers_.find(active_manager_);
  if (mgr_pair != managers_.end())
  {
    ...
    if (target != targets_.end())
    {
      if (SetDataOnTarget(mgr_pair->second.manager, target->second.prot_info, E120_RESET_DEVICE))
      {
...
void LlrpManagerExample::GetInterfaceList(int target_handle)
{
  ...
  if (mgr_pair != managers_.end())
  {
    ...
    {
      auto response_data =
          GetDataFromTarget(mgr_pair->second.manager, target->second.prot_info, E137_2_LIST_INTERFACES);

      if (response_data.size() <= 0xe6)
      {
        const uint8_t* cur_ptr = response_data.data();
        printf("List interfaces:\n");

        while (static_cast<size_t>(cur_ptr - response_data.data()) < response_data.size())
        {
...
void LlrpManagerExample::SetZeroconf(int target_handle, unsigned interface_id)
{
  ...
  if (mgr_pair != managers_.end())
  {
    ...
    {
      uint8_t data_buf[5];
      etcpal_pack_u32b(data_buf, static_cast<uint32_t>(interface_id));
      data_buf[4] = 1;

      if (SetDataOnTarget(mgr_pair->second.manager, target->second.prot_info, E137_2_IPV4_ZEROCONF_MODE, data_buf, 5))
      {
...
void LlrpManagerExample::SetFactoryDefaults(int target_handle)
{
  ...
  if (mgr_pair != managers_.end())
  {
    ...
    if (target != targets_.end())
    {
      if (SetDataOnTarget(mgr_pair->second.manager, target->second.prot_info, E120_FACTORY_DEFAULTS))
      {
...
void LlrpManagerExample::SetComponentScope(int                     target_handle,
                                           int                     scope_slot,
                                           ...
                                           const etcpal::SockAddr& static_config)
{
  ...
  if (mgr_pair != managers_.end())
  {
    ...
    if (target != targets_.end())
    {
      ...
      cur_ptr += 2;
      strncpy((char*)cur_ptr, scope_utf8.c_str(), E133_SCOPE_STRING_PADDED_LENGTH);
      cur_ptr += E133_SCOPE_STRING_PADDED_LENGTH;
...
std::vector<uint8_t> LlrpManagerExample::GetDataFromTarget(llrp::Manager&                manager,
                                                           const llrp::DiscoveredTarget& target,
                                                           uint16_t                      param_id,
                                                           const uint8_t*                data,
                                                           uint8_t                       data_len)
...
bool LlrpManagerExample::SetDataOnTarget(llrp::Manager&                manager,
                                         const llrp::DiscoveredTarget& target,
                                         uint16_t                      param_id,
                                         const uint8_t*                data,
                                         uint8_t                       data_len)
{
  ...
  auto seq_num = manager.SendSetCommand(target.address(), param_id, data, data_len);
  if (seq_num)
  {
    etcpal::Timer resp_timer(LLRP_TIMEOUT_MS);
    while (!resp_timer.IsExpired())
    ...
    if (response.IsValid())
    {
      if (response.seq_num() == *seq_num && response.IsSetResponse() && response.param_id() == param_id)
      {
        // We got a response.
        if (response.IsAck())
          return true;
