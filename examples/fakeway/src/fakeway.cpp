...
bool Fakeway::Startup(const rdmnet::Scope& scope_config, etcpal::Logger& logger, const etcpal::Uuid& cid)
{
  ...
  auto res = rdmnet::Init(logger);
  if (!res)
  {
    log_->Critical("Fatal: couldn't initialize RDMnet library due to error: '%s'", res.ToCString());
    return false;
  }

  auto my_settings = rdmnet::Device::Settings(cid, rdm::Uid::DynamicUidRequest(0x6574));
  my_settings.response_buf = def_resp_->response_buf();
  res = rdmnet_.Startup(*this, my_settings, scope_config);
  if (!res)
  {
    rdmnet::Deinit();
    log_->Critical("Fatal: couldn't start RDMnet device due to error: '%s'", res.ToCString());
    return false;
  }

  if (!gadget_.Startup(*this, logger))
  {
    log_->Critical("Fatal: Failed to start Gadget DLL.");
    rdmnet_.Shutdown();
    rdmnet::Deinit();
    return false;
  }
...
rdmnet::RdmResponseAction Fakeway::ProcessDefRespRdmCommand(const rdm::CommandHeader& cmd_header,
                                                            const uint8_t*            data,
                                                            uint8_t                   data_len)
{
...
void Fakeway::HandleNewRdmResponderDiscovered(unsigned int         gadget_id,
                                              ...
                                              const RdmDeviceInfo& info)
{
  ...
  if (dev_pair != physical_endpoint_rev_lookup_.end() && port_number <= dev_pair->second.size())
  {
    ...
    if (endpt_pair != physical_endpoints_.end())
    {
      ...
      {
        // TODO get real control field and binding UID
        rdmnet_.AddPhysicalResponder(endpt_pair->first, responder, 0);
      }
    }
  }
}

void Fakeway::HandleRdmResponse(unsigned int    gadget_id,
                                unsigned int    port_number,
                                const RDM_CmdC& cmd,
                                const void*     cookie)
{
  ...
  if (dev_pair != physical_endpoint_rev_lookup_.end() && port_number <= dev_pair->second.size())
  {
    ...
    if (endpt_pair != physical_endpoints_.end())
    {
      ...
      {
        rdmnet_.SendRdmUpdate(rdmnet::SourceAddr(endpt_pair->first, resp_src_uid, cmd.getSubdevice()),
                              cmd.getParameter(), reinterpret_cast<const uint8_t*>(cmd.getBuffer()), cmd.getLength());
      }
...
void Fakeway::HandleRdmTimeout(unsigned int    gadget_id,
                               unsigned int    port_number,
